...
<div class="offcanvas-menu-wrapper">
    ...
    <nav class="canvas-menu mobile-menu">
        <ul>
                ...
                <ul class="dropdown">
                    ...
                    ?>
                        ...
                        if ($role == 'trainer' || $role == 'dietician') {
                            ?><li><a href="./customers.php">My Customers</a></li><?php
                        }
            ...
            <li class="<?php if (isset($ot)) { echo $ot; }?>"><a>Other</a>
                <ul class="dropdown">
                                ...
                                <div class="top-option">
                                    ...
                                    <div class="action">
                                        ...
                                        <div class="menu">
                                            ...
                                            <?php
                                            if ($role == 'trainer' || $role == 'dietician') {
                                                ?><li><img src="img/l_icons/users.png"><a href="./customers.php">My Customers</a></li><?php
                                            }
                                            if ($role == 'customer' || $role == 'contributor') {
                                            $user_id = $_SESSION['userid'];
                                            $sql = "SELECT Customer_Id FROM `customer` WHERE `User_Id`='$user_id'";
...
<script>
    function start(title, msg, img){
        switch (img) {
            case 1:
                img = "./img/icons/icon1.jpg";
                break;
            case 2:
                img = "./img/icons/icon2.jpg";
                break;
            case 3:
                img = "./img/icons/icon3.jpg";
                break;
        }
        Push.create(title, {
            body: msg,
            ...
            if($query) {
                ...
                if ($query1) {
                    if(mysqli_num_rows($query1) > 0) {
                        ...
                        $query6 = mysqli_query($conn, "UPDATE `dietfood_routine` SET Sent='0' WHERE Customer_Id='$customer_id' && Time > '".date('H:i:s')."'");

                        $sql2 = "SELECT * FROM `dietfood_routine` WHERE `Customer_Id`='$customer_id' && Time < '".date('H:i:s')."' && Sent='0'";
                        $query2 = mysqli_query($conn, $sql2);
                        if ($query2) {
                            if(mysqli_num_rows($query2)>0) {
                                while ($row1 = mysqli_fetch_array($query2)) {
                                    $sql3 = "SELECT * FROM `dietfood` WHERE Dietfood_Id='".$row1['Dietfood_Id']."'";
                                    $query3 = mysqli_query($conn, $sql3);
                                    $row2 = mysqli_fetch_array($query3);
                                    echo '<script>start("'.$row['Name'].'!","It\'s Time to get '.$row2['Food_Name'].'",'.$row2['Category_Id'].');</script>';
                                    $query4 = mysqli_query($conn, "UPDATE `dietfood_routine` SET Sent='1' WHERE Dietfood_Id = '".$row1['Dietfood_Id']."' && Customer_Id='$customer_id'");
                                }
                            }
                        }
                    }
