...
<head>
    ...
    <meta charset="UTF-8">
    <meta name="description" content="Fitzy Login">
...
<body>
    ...
    if (isset($_POST['btn_submit'])) {
        ...
        if (mysqli_num_rows($query) == 1) {
            ...
            $sql = "INSERT INTO `user_activity`(`User_Activity_Id`, `User_Id`, `Activity_Name`, `IP_Address`, `Activity_Date`, `Activity_Time`) VALUES (NULL,'$user_id','logged in','$user_ip_address','$date','$time')";
            $query =  mysqli_query($conn, $sql);
            ...
            if($role=='admin'){
                ...
                $sql1 = "SELECT * FROM `admin` WHERE User_Id='$user_id'";
                $query1 =  mysqli_query($conn, $sql1);
                if (mysqli_num_rows($query1) == 1) {
                    $result1 = mysqli_fetch_array($query1);
                    $_SESSION['Admin_Id'] = $result1['Admin_Id'];
                    $_SESSION['Name'] = $result1['Name'];
                }
                echo '<script>if (swal("Login Successful!", "You have Logged in successfully to Fitzy", "success")) {setTimeout(function () { window.location.href="./gym/index.php?page=home"; }, 2000); }</script>';
                ...
                <form class="login100-form validate-form" method="POST">
                    ...
                    <div class="wrap-input100">
                        <select name="role" id="role" class="input100 validate-input" required>
                            ...
                            <option value="admin" <?php if ($rr == 'admin') { echo "selected='selected'"; } ?>>Administrator</option>
                        </select>
                        <span class="focus-input100"></span>
                        <span class="symbol-input100">
